...
class BatchedGPUQPSolver:
    ...
    @staticmethod
    @torch.no_grad()
    def solve_mono_multi_g_all(solver: 'BatchedGPUQPSolver',
                               H: np.ndarray,
                               g_list: List[np.ndarray],
                               C: np.ndarray,
                               l: np.ndarray,
                               u: np.ndarray):
        """Solve K monolithic QPs (shared H/C/l/u, per-α g) and return ALL K best solutions.

        This is the monolithic (full-DOF) counterpart of solve_pair_multi_g_all.
        The batch B is partitioned into K groups, one per g vector.  Returns
        the best solution *per group* so the caller can evaluate feasibility /
        progress for each α independently.

        Args:
            solver:   BatchedGPUQPSolver instance.
            H:        (n, n) shared Hessian.
            g_list:   List of K (n,) linear cost vectors (one per α level).
            C:        (m, n) shared constraint matrix.
            l, u:     (m,) shared lower / upper bounds.
        Returns:
            list of K (n,) numpy float32 arrays — best dq per α level.
        """
        n_lin = len(g_list)
        n = H.shape[0]
        m = C.shape[0]
        ...
        # ── Transfer shared matrices to GPU ──────────────────────────────
        H_t = torch.as_tensor(np.ascontiguousarray(H), dtype=dt, device=dev)
        C_t = torch.as_tensor(np.ascontiguousarray(C), dtype=dt, device=dev)
        l_t = torch.as_tensor(np.ascontiguousarray(l), dtype=dt, device=dev).clamp(min=-1e8)
        u_t = torch.as_tensor(np.ascontiguousarray(u), dtype=dt, device=dev).clamp(max=1e8)
        ...
        for k in range(n_lin):
            s = k * per_lin
            e = s + per_lin if k < n_lin - 1 else B
            g_t_k = torch.as_tensor(
                np.ascontiguousarray(g_list[k]), dtype=dt, device=dev)
            g_tensors.append(g_t_k)
            x_offset[s:e] = (-g_t_k) @ K_inv
        ...
        for k in range(n_lin):
            ...
            best_k = torch.argmin(total_k)
            x_best_k = x_k[best_k]
            results.append(x_best_k.cpu().numpy().astype(np.float32))
